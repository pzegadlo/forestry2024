...
data <- read_csv("GLMM_Input_Af_2024-06-25.csv", col_types = cols(
...
# Baseline GLMM
model_1_baseline <- glmer(formula = captures ~ 1 + VegH + UndD + SMI_4 + CWDc + CWDd45 +
                                 Plnt_1 + Plnt_2 + Plnt_3 + Plnt_4 + Plnt_5 + Plnt_8 + Plnt_11 + Plnt_12 +
                                 PopAf + PopCg +
                                 Year_2018 + Year_2019 +
                                 I_VegH2_UndD + I_VegH3_UndD +
                                 I_VegH2_CWDc + I_VegH3_CWDc + I_UndD2_CWDc + I_UndD3_CWDc + 
                                 I_Yr_StS_T + I_Yr_StS_R + I_Yr_StS_S + 
                                 I_PopHi_VegH_2 + I_PopHi_VegH_3 + I_PopHi_UndD + I_PopHi_CWDc +
...
overdisp.glmer(model_baseline)

# No plant species groups
model_2 <- glmer(formula = captures ~ 1 + VegH + UndD + SMI_4 + CWDc + CWDd45 +
                            PopAf + PopCg +
                            Year_2018 + Year_2019 +
                            I_VegH2_UndD + I_VegH3_UndD +
                            I_VegH2_CWDc + I_VegH3_CWDc + I_UndD2_CWDc + I_UndD3_CWDc + 
...
# No plant species groups + no humidity
model_3 <- glmer(formula = captures ~ 1 + VegH + UndD + CWDc + CWDd45 +
                   PopAf + PopCg +
                   Year_2018 + Year_2019 +
                   I_VegH2_UndD + I_VegH3_UndD +
                   I_VegH2_CWDc + I_VegH3_CWDc + I_UndD2_CWDc + I_UndD3_CWDc + 
...
# No plant species groups + no humidity + only significant controls and interactions
model_4 <- glmer(formula = captures ~ 1 + VegH + UndD + CWDc + CWDd45 +
                   PopAf +
                   Year_2018 +
                   I_VegH2_CWDc + I_Yr_2019_StS_T +
                   (1 | SquareID), data = data, family = poisson,
                 control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=7e5)))  
...
data_tab <- data.frame(cbind(res, data$cwd_pc))
colnames(data_tab) <- c("res", "cwd_pc")
data_tab$cwd_pc <- round_any(data_tab$cwd_pc, 0.5)
data_tab$cwd_pc[data_tab$cwd_pc > 10.0] <- 10.0
data_chart <- data_tab %>% 
  group_by(cwd_pc) %>% 
  summarize(mean = mean(res), cnt = n())
ch_preprog = ggplot(data_chart, aes(x=cwd_pc, y=mean)) + 
  geom_point(aes(size = cnt)) + geom_hline(yintercept=0) + theme_bw() + 
  scale_x_continuous(limits=c(0,10), breaks=seq(0, 10, 1), labels = c(0:9, "10+")) +
  scale_y_continuous(limits=c(-0.3,0.3), breaks=seq(-0.3, 0.3, 0.1), labels = scales::label_number(accuracy = 0.1)) +
  labs(x = bquote(atop('Coarse woody debris coverage'~(m^2),"No CWD threshold")), y = "Mean Pearson residual", size = "Observation count") +
  theme(legend.position="bottom") + scale_size_area() +
  geom_rect(aes(xmin = 0.75, xmax = 1.75, ymin = -0.175, ymax = 0.1), 
            fill = "gray", alpha = 0.01, color = "black", linetype = 'dotted')


# Final choice

data$CWDc_T <- (data$cwd_pc > 1.5)

# Model 4 with added threshold
model_final <- glmer(formula = captures ~ 1 + VegH + UndD + CWDc + CWDd45 + CWDc_T +
                   PopAf +
                   Year_2018 +
                   I_VegH2_CWDc + I_Yr_2019_StS_T +
                   (1 | SquareID), data = data, family = poisson,
...
data_chart <- data_tab %>% 
  group_by(cwd_pc) %>% 
  summarize(mean = mean(res), cnt = n())
ch_postprog = ggplot(data_chart, aes(x=cwd_pc, y=mean)) + 
  geom_point(aes(size = cnt)) + geom_hline(yintercept=0) + theme_bw() + 
  scale_x_continuous(limits=c(0,10), breaks=seq(0, 10, 1), labels = c(0:9, "10+")) +
  scale_y_continuous(limits=c(-0.3,0.3), breaks=seq(-0.3, 0.3, 0.1), labels = scales::label_number(accuracy = 0.1)) +
  labs(x = bquote(atop('Coarse woody debris coverage'~(m^2),"With CWD threshold")), y = "", size = "Observation count") +
  theme(legend.position="bottom") + scale_size_area()

p_prog <- ggarrange(ch_preprog, ch_postprog, nrow = 1, common.legend = TRUE, legend="bottom")
#ggsave("Af_Threshold.png", plot = p_prog, device = "png", width =9, height =6)
